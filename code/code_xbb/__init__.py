        ...
        if (f.split(".")[-1] == 'txt'):  # 先只处理dw数据
            ...
            for i in target_dic["entity"]:
                for j in target_dic["entity"][i]:
                    raw_info.append([str]+list(i) + j)

    df = pd.DataFrame(raw_info)
    df.columns = ["file","entity_id", "entity_type", "entity_specificity", "mention_text",
...
if __name__ == "__main__":
    ...
    logging.info('**********get num feature*********')
    TrainX, TrainY,cata_feature_num = get_feature()

    print TrainX.shape
    print TrainY.shape
    logging.info('*********train model**************')
    train_test(TrainX,TrainY,cata_feature_num)
