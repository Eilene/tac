...
def read_file_info_records(ere_dir, entity_info_dir, relation_info_dir, event_info_dir, em_args_dir, both_genre=True):
    df_records = []
    nw_records = []
    records = []
    ...
    for parent, dirnames, ere_filenames in os.walk(ere_dir):
        for ere_filename in ere_filenames:  # 输出文件信息
            ...
            if record != {}:
                if both_genre is True:
                    if record['filename'][:3] != 'AFP' and record['filename'][:3] != 'APW' \
                            and record['filename'][:3] != 'NYT' and record['filename'][4:6] != 'NW':  # 论坛数据
