...
from src.chinese_sentiment.utils.constants import *
from src.chinese_sentiment.utils.split_sentences import *
...
def extract_event_each_file(source_filepath, ere_filepath, annotation_filepath, part_name, with_none):
    ...
    for i in range(len(hopper_list)):
        ...
        for j in range(len(event_mention_list)):
            ...
            if trigger_context_dict is None:
                print trigger_text, trigger_offset, part_name
                # 只有一个“买”，在标签里，是用户名，这怎么作为事件的。。应该是predict ere的错误识别
                # 买 4167 CMN_DF_000311_20151122_G00A0EQRF
            # 拼成一个字符串
            ...
            # 从上下文中进一步提取窗口词
            if trigger_context_dict is not None:
                window_length = 10
