...
def gen_general_features(file_records):
    ...
    for context in contexts:
        ...
        for word in words:
            ...
            # print token_text
        tokenized_contexts.append(token_text)
    # 再tfidf
    vec = TfidfVectorizer(min_df=1, ngram_range=(1, 1), max_features=300, binary=True)  # 不加限制共104634个词
    # vec.fit_transform(tokenized_contexts)
    tfidf_features = vec.fit_transform(tokenized_contexts).toarray()
    import json
    res_data = json.dumps(vec.get_feature_names(), ensure_ascii=False, encoding="gb2312")
    print res_data
    tfidf_features = tfidf_features.tolist()
    # 词性计数，情感词计数
...
def pos_senti_count(texts):
    ...
    for i in range(len(texts)):
        ...
        # neg_senti_count = 0
        for p in pos:
            if p.flag in pos_name:
                pos_count[pos_name.index(p.flag)] += 1
            else:
                pos_name.append(p.flag)
                pos_count.append(1)
