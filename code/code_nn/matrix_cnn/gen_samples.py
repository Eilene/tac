...
def gen_train_sample(records, embeddings_index, dim):
    ...
    for rec in records:
        ...
        # 组成词向量矩阵并截断补齐
        word_matrix = []
        row_num = 50  # 截断补齐长度，即矩阵行数
        word_num = len(words_filtered)
        for word in words_filtered:
            embedding_vector = embeddings_index.get(word)
            if embedding_vector is not None:
                word_matrix.append(embedding_vector)
            else:
                word_num -= 1
        if word_num > row_num:  # 需截断
            word_matrix = word_matrix[:50]
        elif word_num < row_num:  # 需补齐
            zero_vec = [0.0] * dim
            for i in range(row_num-word_num):
                word_matrix.append(zero_vec)
        # 补上有关target本身的特征，这里先用词向量
        target_words = WordPunctTokenizer().tokenize(rec['target']['text'])
        target_words_filtered = [word for word in target_words if not word in english_punctuations]
        word_vec = [0.0]*dim
        word_count = 0
        for word in target_words_filtered:
            embedding_vector = embeddings_index.get(word)
            if embedding_vector is not None:
                word_count += 1
                for i in range(dim):
                    word_vec[i] += embedding_vector[i]
        if word_count != 0:
            for i in range(dim):
                word_vec[i] /= word_count
        word_matrix.append(word_vec)
        # 词向量矩阵加入样本中
        x_samples.append(word_matrix)
    ...
    for rec in records:
        ...
        for word in words_filtered:
            ...
            if embedding_vector is not None:
                ...
                word_num -= 1
        if word_num > row_num:  # 需截断
            word_matrix = word_matrix[:row_num]
        elif word_num < row_num:  # 需补齐
            zero_vec = [0.0] * dim
            for i in range(row_num - word_num):
                word_matrix.append(zero_vec)
