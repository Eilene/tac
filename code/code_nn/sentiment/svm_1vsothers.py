...
if __name__ == '__main__':
    ...
    x_train1, y_train1 = resampling(x_train, y_train1, (negnum+posnum)*2)
    ...
    joblib.dump(clf1, '1vsothers_svm_model1.m')  # 保存训练模型
    clf2 = svm.SVC(probability=True)
    clf2.fit(x_train2, y_train2)
    joblib.dump(clf2, '1vsothers_svm_model2.m')  # 保存训练模型
    clf3 = svm.SVC(probability=True)
    clf3.fit(x_train3, y_train3)
    joblib.dump(clf3, '1vsothers_svm_model3.m')  # 保存训练模型
    
    # 测试
    print 'Test...'
    clf1 = joblib.load('1vsothers_svm_model1.m')
    y_pred_proba1 = clf1.predict_proba(x_test)
    y_predict1 = predict_by_proba_nothreshold(y_pred_proba1)
    clf2 = joblib.load('1vsothers_svm_model2.m')
    y_pred_proba2 = clf2.predict_proba(x_test)
    y_predict2 = predict_by_proba_nothreshold(y_pred_proba2)
    clf3 = joblib.load('1vsothers_svm_model3.m')
